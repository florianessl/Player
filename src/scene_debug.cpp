...
void Scene_Debug::DoCallCommonEvent() {
	...
	if (Game_Battle::IsBattleRunning()) {
		Game_Battle::GetInterpreter().Push(&ce, Game_Interpreter::ePush_Debug);
		...
		Game_Map::GetInterpreter().Push(&ce, Game_Interpreter::ePush_Debug);
		Scene::PopUntil(Scene::Map);
...
void Scene_Debug::DoCallMapEvent() {
	...
	if (Game_Battle::IsBattleRunning()) {
		Game_Battle::GetInterpreter().Push(me, page, false, Game_Interpreter::ePush_Debug);
		Scene::PopUntil(Scene::Battle);
		Output::Debug("Debug Scene Forced execution of map event {} page {} on the battle foreground interpreter.", me->GetId(), page->ID);
	} else {
		Game_Map::GetInterpreter().Push(me, page, false, Game_Interpreter::ePush_Debug);
		Scene::PopUntil(Scene::Map);
...
void Scene_Debug::DoCallBattleEvent() {
	...
	Game_Battle::GetInterpreter().Push(page.event_commands, 0, 0, false, lcf::rpg::SaveEventExecState::EasyRpgTrigger_called, Game_Interpreter::ePush_Debug);
	Scene::PopUntil(Scene::Battle);
