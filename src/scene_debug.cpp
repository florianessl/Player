...
void Scene_Debug::DoCallCommonEvent() {
	...
	if (Game_Battle::IsBattleRunning()) {
		Game_Battle::GetInterpreter().Push(&ce, Debug::ePush_Debug);
		...
		Game_Map::GetInterpreter().Push(&ce, Debug::ePush_Debug);
		Scene::PopUntil(Scene::Map);
...
void Scene_Debug::DoCallMapEvent() {
	...
	if (Game_Battle::IsBattleRunning()) {
		Game_Battle::GetInterpreter().Push(me, page, false, Debug::ePush_Debug);
		Scene::PopUntil(Scene::Battle);
		Output::Debug("Debug Scene Forced execution of map event {} page {} on the battle foreground interpreter.", me->GetId(), page->ID);
	} else {
		Game_Map::GetInterpreter().Push(me, page, false, Debug::ePush_Debug);
		Scene::PopUntil(Scene::Map);
...
void Scene_Debug::DoCallBattleEvent() {
	...
	Game_Battle::GetInterpreter().Push(page.event_commands, 0, 0, false, Debug::eTrigger_called, Debug::ePush_Debug);
	Scene::PopUntil(Scene::Battle);
